...
const Lideres = () => {
    ...
    useEffect(() => {
        const fetchSupervisores = async () => {
            try {
                const response = await fetch(`${api}list_supervisores?mat_gestor=${cookies.login}`);
    ...
    const fetchPerguntas = async () => {
        try {
            const response = await fetch(`${api}questoes?tipo=3&setor=${cookies.cargo}`);
            const data = await response.json();
    ...
    const handleSupervisorSelect = async (login: string) => {
        ...
        const supervisorSelecionado = supervisorOptions.find(s => s.login === login);
        ...
        try {
            ...
            fetchPerguntas(); // Recarregar perguntas para o novo supervisor
        } catch (error) {
                ...
                <div className="supervisor-selection">
                    ...
                    >
                        ...
                        {supervisorOptions.map((supervisor) => (
                            ...
                            >
                                {supervisor.nome} {supervisor.avaliado ? '(Avaliado)' : '(Pendente)'}
                            </option>
                        ))}
                    </select>
                </div>
                ...
                {selectedSupervisor && !selectedSupervisor.avaliado ? (
                    <>
                        <div className="selected-supervisor">
                            ...
                            <span className="matricula">Matrícula: {selectedSupervisor.matricula}</span>
                        </div>
                        ...
                        <div className="comentario-container">
                            ...
                            <input className="comentario-input" type="text" placeholder="Melhorias" value={negativos} onChange={(e) => setNegativos(e.target.value)} />
