...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchStatusTipo2 = async () => {
      const response = await fetch(`${api}status_mes?matricula=${logado.matricula}&tipo=2`);
      const data: StatusMes = await response.json();
      setStatusTipo2(data.data.status);
    };
    const fetchStatusTipo1 = async () => {  
      const response = await fetch(`${api}status_mes?matricula=${logado.matricula}&tipo=1`);
      const data: StatusMes = await response.json();
