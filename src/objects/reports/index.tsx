...
const Reports: React.FC = () => {
	...
	const fetchReportData = async () => {
		setLoading(true);
		try {
			const response = await fetch(`${api}relatorios/geral?mes=${mesSelecionado}`);
		...
		<div className="reports-container">
			<div className="reports-header">
				...
				<div className="reports-filters">
					<select className="period-select" onChange={async (e) => {
						setMesSelecionado(e.target.value);
					}}>
						<option value="">Selecione um mês</option>
						{meses.map((mes) => (
							<option key={mes.key} value={mes.value}>{mes.value}</option>
						))}
					</select>
				</div>
			</div>
